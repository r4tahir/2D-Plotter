...
void sortArrays(int size)
{
    ...
    displayBigTextLine(2, "Sorting...");
    for (line = 0; line < size; line++)
    {
        for (targetNumber = line; targetNumber < size; targetNumber++)
        {
            for (comparison = line; comparison < size; comparison++)
            {
                if (dot[targetNumber][1] < dot[comparison][1])
                    largest = false;
            }
            if (largest == true)
            {
                tempy = dot[line][1];
                tempx = dot[line][0];
                dot[line][1] = dot[targetNumber][1];
                dot[line][0] = dot[targetNumber][0];
                dot[targetNumber][1] = tempy;
                dot[targetNumber][0] = tempx;
                foundLargest = true;
            }
            largest = true;
        }
